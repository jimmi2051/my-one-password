...
from auth.keychain import get_key
...
async def touchid_status(
    ...
    """Return whether Touch ID credential is registered and vault key is in Keychain."""
    credential_exists = (
        db.query(WebAuthnCredential)
        .filter(WebAuthnCredential.user_id == user.id)
        .first()
    ) is not None
    keychain_available = get_key(user.email) is not None
    return TouchIdStatusOut(registered=credential_exists, keychain_available=keychain_available)
...
async def login(
    ...
    """Verify Touch ID assertion and unlock the vault from Keychain."""
    challenge = _pop_challenge(jti)

    # The credential's id field is a base64url string matching what we stored
    ...
    # Retrieve vault key from Keychain
    keychain_hex = get_key(user.email)
    if not keychain_hex:
        # Touch ID authenticated OK but vault key not in Keychain (e.g. new device)
        return WebAuthnLoginOut(
            message="Touch ID verified but vault key not in Keychain",
            email=user.email,
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Corrupted keychain entry")
